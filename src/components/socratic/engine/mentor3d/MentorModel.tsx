...
export function MentorModel({
  ...
  useFrame((_, dt) => {
    ...
    if (b.spine) {
      b.spine.rotation.x = THREE.MathUtils.damp(
        b.spine.rotation.x,
        pose.spineX + breath,
        2,
        dt,
      );
      b.spine.rotation.z = THREE.MathUtils.damp(
        b.spine.rotation.z,
        pose.spineZ + sway,
        2,
        dt,
    ...
    const damp = 2.5;
    safeSet(vrm, VRMExpressionPresetName.Happy, THREE.MathUtils.damp(safeGet(vrm, VRMExpressionPresetName.Happy), tHappy, damp, dt));
    safeSet(vrm, VRMExpressionPresetName.Angry, THREE.MathUtils.damp(safeGet(vrm, VRMExpressionPresetName.Angry), tAngry, damp, dt));
    safeSet(vrm, VRMExpressionPresetName.Sad, THREE.MathUtils.damp(safeGet(vrm, VRMExpressionPresetName.Sad), tSad, damp, dt));
    safeSet(vrm, VRMExpressionPresetName.Relaxed, THREE.MathUtils.damp(safeGet(vrm, VRMExpressionPresetName.Relaxed), tRelaxed, damp, dt));
    safeSet(vrm, VRMExpressionPresetName.Neutral, THREE.MathUtils.damp(safeGet(vrm, VRMExpressionPresetName.Neutral), tNeutral, damp, dt));
    safeSet(vrm, VRMExpressionPresetName.Surprised, THREE.MathUtils.damp(safeGet(vrm, VRMExpressionPresetName.Surprised), tSurprised, damp, dt));
    ...
    if (isSpeaking && !isPausing) {
      ...
      safeSet(vrm, VRMExpressionPresetName.Aa, THREE.MathUtils.damp(safeGet(vrm, VRMExpressionPresetName.Aa), 0, 10, dt));
      safeSet(vrm, VRMExpressionPresetName.Ih, THREE.MathUtils.damp(safeGet(vrm, VRMExpressionPresetName.Ih), 0, 10, dt));
      safeSet(vrm, VRMExpressionPresetName.Ou, THREE.MathUtils.damp(safeGet(vrm, VRMExpressionPresetName.Ou), 0, 10, dt));
    }
  });
